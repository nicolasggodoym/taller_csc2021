...
Ahora, ocupamos la función `case_when()` para que crear una nueva variable llamada `macrozona`, agrupando a la variable región en Norte (15, 1, 2 y 3); Centro (4, 5 y 13); Centro sur (6, 7, 16 y 8); Sur (9, 10 y 14) Austral (11, 12). Recuerda usar el operador or (|), y englobar todo en `mutate()`

```{r case_when}
data <- data %>% 
  mutate(macrozona = case_when(region == 15|region == 1 | region == 2 | region == 3 ~ 'Norte',
            region == 4 | region == 5 | region == 13 ~ 'Centro',
            region == 6 | region == 7 | region == 16 | region == 8 ~ 'Centro sur',
            region == 9 | region == 10 | region == 14 ~ 'Sur',
                        region == 11 | region == 12 ~ 'Austral'))
```

Ahora, transforma la variable macrozona en factor con la función `factor()`, estableciendo sus levels como Norte, Centro, Centro sur, Sur y Austral

```{r factor_macrozona}
data$macrozona <- factor(data$macrozona, levels = c('Norte',
                                                    'Centro',
                                                    'Centro sur',
                                                    'Sur',
                                                    'Austral'))
...
¡Realicemos algunos cálculos! Calculemos la media de edad para cada macrozona. Para ello, emplearemos las funciones `group_by()` y `summarise()`. ¡No olvides los pipes!
...
¡Excelente! Ahora la media del Estatus social subjetivo y la edad según sexo. ¡No olvides remover los valores nulos del cálculo!

```{r ess_sex}
data %>% 
  group_by(sexo) %>% 
  summarise("Estatus social subjetivo" = mean(ess, na.rm=T),
            Edad = mean(edad))
```

Ahora lo haremos algo más complejo. Agrupando por Clase social subjetiva, revisemos las frecuencias de la percepción de conflicto. Para ello, emplearemos `group_by()` y `frq()`
